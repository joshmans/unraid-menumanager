...
- A page can list several menus separated by spaces. Only the first token may be indirect: `/file KEY=default` reads `KEY` from an ini file, `$var default` evaluates a PHP variable. Either way the two tokens become one resolved menu and the remaining tokens follow.
...
- **Move / reorder:** rewrite `Menu=` to `Category:rank` (ranks are 10, 20, 30…). Only the category token is replaced, so `Utilities Buttons` keeps its button. For an indirect menu the whole indirection is replaced by the fixed token, and the `;mm-orig:` marker brings it back on reset.
